...
try {
    ...
    if (
        j04.result !== "A-S_PASS" ||
        [j05, j06, j07, j08, j09, j10, j11].some(
            (value) => value.result !== "A-T_PASS",
        )
    )
    ...
    const result = {
        ...
        verdict:
            "Evidence only. This verifier does not merge, tag, start J1 or independently issue the owner-controlled J0 Foundation v1 GO.",
    };

    await writeFile(
        new URL(".jarvis/acceptance/j12-foundation-v1.json", root),
        ...
        await appendFile(
            ...
            "\n## J0.12 Foundation v1 A–T acceptance\n\n" +
                Object.entries(phases)
                    .map(
                        ([letter, phase]) =>
                            `| ${letter} | ${phase.result} | ${phase.assertions} assertion(s) |`,
                    )
                    .join("\n") +
