...
are retained. Expired unapproved enrollment and its unused public credentials are
removed so unauthenticated enrollment cannot permanently occupy device slots.
Canonical public-key encoding prevents alternate encodings from bypassing key-reuse
checks. This is not a high-throughput or multi-tenant identity database.
