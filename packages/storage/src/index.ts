...
export function databasePool(
    config: JarvisConfig["storage"]["postgres"],
    password: string,
    migrator = false,
): pg.Pool {
    const pool = new pg.Pool({
        ...
        user: migrator ? config.migratorUser : config.runtimeUser,
        password,
        max: 4,
...
export class PostgresMemoryRepository implements MemoryRepository {
    ...
    async save(input: MemoryRecord): Promise<void> {
