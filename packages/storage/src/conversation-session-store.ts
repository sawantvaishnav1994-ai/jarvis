  ...
  switch (postgresErrorCode(error)) {
    case "23503": {
      ...
      if (constraint?.endsWith("owner_id_fkey"))
        return new BoundaryError("CONVERSATION_OWNER_REFERENCE_INVALID");
      if (constraint?.endsWith("device_id_fkey"))
        return new BoundaryError("CONVERSATION_DEVICE_REFERENCE_INVALID");
...
export class PostgresConversationSessionRepository {
  ...
  async createSession(session: Session): Promise<Session> {
    try {
      ...
      const found = existing.rows[0] ? sessionFrom(existing.rows[0]) : null;
