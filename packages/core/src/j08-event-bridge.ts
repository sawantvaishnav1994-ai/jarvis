...
export class GovernedEventToolConsumer implements EventConsumer{
  ...
  async handle(event:JarvisEventEnvelope){const request=this.requestFor(event);await this.gateway.invoke({...request,source:"SYSTEM",metadata:{...request.metadata,eventId:event.eventId,correlationId:event.correlationId}});}
}
