...
// Explicit v2: old readers must not silently treat a policy decision as a v1 entry.
export const PolicyAuditRecordSchema = z
    .strictObject({
        ...
        policyRevision: IdentifierSchema,
        reason: IdentifierSchema,
        controlEpoch: z.number().int().nonnegative(),
        risk: z.enum(["low", "sensitive", "critical"]),
        matchedRuleIds: z.array(IdentifierSchema).max(128),
        timestamp: z.iso.datetime(),
    })
    .refine(
        (v) => v.environment === v.actor.environment,
...
export const StoredAuditRecordSchema = z.union([
    AuditRecordSchema,
