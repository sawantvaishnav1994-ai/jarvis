...
export const EventSchema = z
    .strictObject({
        ...
        sensitivity: PrivacySchema,
        correlationId: IdentifierSchema,
    })
    .refine(
        (e) => e.actor.environment === e.environment,
        "Event environment mismatch",
    );
export type JarvisEvent = z.infer<typeof EventSchema>;
export interface EventPublisher {
    publish(event: JarvisEvent): Promise<void>;
}
export const FoundationJobSchema = z.strictObject({
    version: ContractVersionSchema,
    type: z.literal("foundation.ping"),
    environment: z.literal("development"),
    correlationId: z.uuid(),
});
export type FoundationJob = z.infer<typeof FoundationJobSchema>;
export const queueName = (environment: string) =>
    "jarvis-" + EnvironmentSchema.parse(environment) + "-foundation";
