...
export const MemoryRecordSchema = z
    .strictObject({
        ...
        kind: z.enum(["working","conversation","episodic","semantic","project","preference","procedural","relationship","device"]),
    ...
    .refine((v) => v.retention !== "temporary" || v.expiresAt !== null,"Temporary memory needs expiry");
...
export class MemoryService {
    ...
    async remember(actor: Actor, input: unknown): Promise<void> {
        ...
        if (actor.kind !== "owner" || actor.id !== record.ownerId || record.retention === "never-store") throw new BoundaryError("MEMORY_DENIED");
    ...
    async recall(actor: Actor, projectId: string): Promise<MemoryRecord[]> {
        ...
        return (await this.repository.find(actor.id,IdentifierSchema.parse(projectId))).filter((r) => !r.expiresAt || Date.parse(r.expiresAt) > Date.now());
    }
}
