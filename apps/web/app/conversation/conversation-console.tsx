...
export function ConversationConsole() {
    ...
    async function send() {
        ...
        try {
            ...
            setStatus("Device proof verified. Running governed conversation pipeline…");
            ...
            const code = error instanceof Error ? error.message : "CONVERSATION_DENIED";
            if (["SESSION_INVALID", "SESSION_EXPIRED", "DEVICE_NOT_TRUSTED"].includes(code))
                setStatus(`${code}. Re-authenticate in Identity.`);
            else if (["SAFE_MODE", "FREEZE", "SHUTDOWN", "SECURITY_LOCKDOWN"].includes(code))
                setStatus(`${code}. Conversation authority is restricted by emergency control.`);
            else setStatus(code);
        } finally {
            setBusy(false);
        }
    ...
    return (
        <section className="conversation-console" aria-label="JARVIS conversation">
            <div className="conversation-status" role="status" aria-live="polite">
                {status}
            </div>
            <div className="conversation-meta" aria-label="Conversation authority indicators">
                ...
                <span>External AI: {turn?.privacy.externalAI ? "enabled" : "off"}</span>
                <span>Storage: {turn?.privacy.stored ? "stored" : "session-only"}</span>
            </div>
                ...
                {turn ? (
                    <article>
                        ...
                        <p className="conversation-response">
                            {turn.response ?? "No response content was committed."}
                        </p>
                        <dl className="conversation-details">
                            ...
                            <div>
                                <dt>Approval</dt>
                                <dd>{turn.approval?.status ?? "not requested"}</dd>
                            </div>
                            <div>
                                <dt>Tool</dt>
                            ...
                            <ol className="conversation-events">
                                {turn.events.map((event) => (
                                    <li key={`${event.sequence}:${event.kind}`}>
                                        <code>#{event.sequence}</code> {event.state} / {event.kind}
                    ...
                    <p className="conversation-empty">
                        No turn yet. JARVIS will show response provenance, privacy,
                        approval/tool state and ordered events here.
                    </p>
                )}
            </div>
            ...
            <p className="conversation-boundary">
                J1.11 uses the local synthetic development model and does not execute
                tools or approvals. J1.12 replaces this content-only adapter with the
                complete persistence, memory, tool, approval, audit and recovery chain.
            </p>
        </section>
    );
}
