...
const actor = {version:1 as const,id:"j05-vector-postgres",kind:"service" as const,environment:"development" as const};
...
beforeAll(async()=>{
 ...
 const runtimeLease=await manager.lease(config.storage.postgres.passwordRef,actor),adminLease=await manager.lease(config.storage.postgres.migratorPasswordRef,actor);runtimePassword=runtimeLease.value.toString("utf8");pool=databasePool(config.storage.postgres,runtimePassword);admin=databasePool(config.storage.postgres,adminLease.value.toString("utf8"),true);runtimeLease.destroy();adminLease.destroy();
 await migrate(admin,"infrastructure/migrations","development",config.storage.postgres.runtimeUser,runtimePassword);
 await pool.query(`INSERT INTO identity.root_owner(id,payload) SELECT 'j05-vector-owner','{}' WHERE NOT EXISTS (SELECT 1 FROM identity.root_owner)`);ownerId=(await pool.query<{id:string}>("SELECT id FROM identity.root_owner WHERE singleton=true")).rows[0]!.id;
