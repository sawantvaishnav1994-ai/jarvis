...
const readJson = async <T = Record<string, unknown>>(
    path: string,
): Promise<T> => JSON.parse(await readText(path)) as T;
...
interface DataManifest {
    classes: Array<{
        id: string;
        externalModelEligibility: string;
...
describe("J0.12 Foundation v1 acceptance", () => {
    it("J0.12 A exact accepted baseline and reproducible freeze inputs", async () => {
        const manifest = await readJson<FoundationManifest>(
            "foundation/foundation-v1.manifest.json",
        );
        expect(manifest.baseline).toBe(
            "581badf38ed3d85c75a652ac87111c8aa4e62da9",
        );
    ...
    it("J0.12 B Foundation v1 architecture and trust boundaries are frozen", async () => {
        ...
        expect(architecture).toContain(
            "schema -> proposal -> policy/risk -> approval when needed -> permit -> execution -> result -> audit/event",
        );
    ...
    it("J0.12 C every foundation-critical contract family is explicitly versioned", async () => {
        ...
        expect(
            await readText("docs/contracts/foundation-v1-contracts.md"),
        ).toContain("Change rule after Foundation v1");
    });

    it("J0.12 D schema is frozen at immutable migrations 0001 through 0014", async () => {
        const schema = await readJson<SchemaManifest>(
            "foundation/schema-v1.json",
        );
        const live = await readJson<
            Array<{ version: number; file: string; sha256: string }>
        ...
        expect(schema.migrations).toEqual(
            live.map(({ version, file, sha256: hash }) => ({
                version,
                file,
                sha256: hash,
            })),
        ...
        for (const migration of schema.migrations)
            expect(
                await sha256(`infrastructure/migrations/${migration.file}`),
    ...
    it("J0.12 E Root Owner identity device trust session revocation and recovery stay governed", async () => {
        ...
        expect(contracts).toContain(
            "Browser passkey authentication and independent device proof are distinct signals",
        );
        for (const path of [
    ...
    it("J0.12 F P0 through P5 policy risk approval permit and delegation semantics are frozen", async () => {
        const permissions = await readJson<PermissionManifest>(
            "foundation/permissions-v1.json",
        );
        expect(permissions.levels.map(({ id }) => id)).toEqual([
            "P0",
            "P1",
            "P2",
            "P3",
        ...
        expect(
            permissions.levels.some(({ prohibited }) =>
                prohibited.some((value) => value.includes("self-grant")),
            ),
        ).toBe(true);
        expect(permissions.globalRules.join(" ")).toContain(
    ...
    it("J0.12 G privacy storage encryption D5 retention deletion export secrets and NEVER_STORE are frozen", async () => {
        const data = await readJson<DataManifest>(
            "foundation/data-classification-v1.json",
        );
        expect(data.classes.map(({ id }) => id)).toEqual([
            "D0",
            "D1",
            "D2",
            "D3",
        ...
        expect(d5).toMatchObject({
            externalModelEligibility: "never",
            privacy: "local-only",
        });
        expect(data.retentionModes).toContain("never-store");
        expect(data.neverStore.semanticVersion).toBe(1);
        for (const path of [
            "tests/security/envelope.test.ts",
            "tests/security/storage-boundaries.test.ts",
    ...
    it("J0.12 H memory scope provenance disclosure expiry purge and owner isolation are frozen", async () => {
        ...
        expect(contracts).toContain(
            "owner scoped with optional project scope, provenance/source metadata",
        );
        for (const path of [
            "packages/memory/src/j05-contracts.ts",
            "tests/integration/j05-memory-postgres.test.ts",
            "tests/integration/j05-identity-isolation.test.ts",
        ])
            expect(await exists(path)).toBe(true);
    });
    ...
    it("J0.12 I model providers remain replaceable non-authoritative adapters behind privacy controls", async () => {
        ...
        expect(architecture).toContain(
            "replaceable provider adapters/router with privacy preflight",
        );
        expect(architecture).toContain(
            "model suggestion, event payload, queue message or stored object is never sufficient authorization",
        );
        for (const path of [
            "packages/models/src/j06-contracts.ts",
            "packages/models/src/router.ts",
            "tests/unit/j06-router.test.ts",
        ])
            expect(await exists(path)).toBe(true);
    });
    ...
    it("J0.12 J executable tools remain behind schema proposal policy approval permit and gateway execution", async () => {
        ...
        expect(contracts).toContain(
            "schema validation -> proposal -> policy/risk -> approval when required -> bound execution permit -> execution -> result -> event/audit",
        );
        for (const path of [
            "packages/tools/src/j07-gateway.ts",
    ...
    it("J0.12 K events remain versioned persisted privacy-aware idempotent and constrained through transport workers", async () => {
        ...
        expect(contracts).toContain(
            "Redis/BullMQ transport enforce duplicate/idempotency controls",
        );
        for (const path of [
            "packages/events/src/j08-contracts.ts",
            "packages/events/src/j08-transport.ts",
            "tests/integration/j08-events.test.ts",
            "scripts/j08-queue-smoke.mjs",
        ])
            expect(await exists(path)).toBe(true);
    });

    it("J0.12 L audit remains correlated append-oriented integrity-checked and protected-plaintext safe", async () => {
        const review = await readText(
            "docs/security/foundation-v1-security-review.md",
        );
        expect(review).toContain(
            "append-oriented DB restrictions, keyed integrity chain, checkpoint verification",
        );
        for (const path of [
            "packages/audit/src/j09-integrity.ts",
            "tests/integration/j09-audit-postgres.test.ts",
            "tests/unit/j09-hardening.test.ts",
        ])
            expect(await exists(path)).toBe(true);
    });
    ...
    it("J0.12 M recovery preserves owner revocation deletion policy memory audit events encryption and epochs", async () => {
        ...
        expect(architecture).toContain(
            "preserves a single Root Owner, revocations, deletion obligations, policy restrictions, audit/events, memory and security epoch semantics",
        );
        for (const path of [
            "packages/storage/src/j10-recovery.ts",
            "tests/integration/j10-recovery-postgres.test.ts",
            "scripts/j10-recovery-drill.mjs",
        ])
            expect(await exists(path)).toBe(true);
    });
    ...
    it("J0.12 N adversarial recovery rejects wrong key owner tamper substitution expiry and unsupported versions", async () => {
        ...
        for (const term of [
            "Cross-owner restore",
            "Backup tampering",
            "Stale-authority restore",
            "Schema tamper/future schema",
        ])
    ...
    it("J0.12 O critical dependency failure reduces readiness and never creates authorization", async () => {
        ...
        expect(architecture).toContain(
            "dependency unavailability makes affected readiness/capabilities fail closed rather than permitting work",
        );
    ...
    it("J0.12 P replay concurrency and race-sensitive authority remains one-shot fail-closed and regression-covered", async () => {
        ...
        expect(review).toContain(
            "one-shot approval and permit consumption with action/session/epoch binding",
        );
        expect(await exists("tests/security/governance.test.ts")).toBe(true);
        expect(await exists("tests/security/governed-gateway.test.ts")).toBe(
            true,
        );
    });

    it("J0.12 Q PAUSE FREEZE DISCONNECT SAFE MODE REVOKE and SHUTDOWN only reduce or terminate capability", async () => {
        const modes = await readJson<ModeManifest>(
            "foundation/operating-modes-v1.json",
        );
        expect(modes.controls).toEqual([
            "PAUSE",
            "FREEZE",
            "DISCONNECT",
            "SAFE MODE",
        ...
        expect(
            modes.modes.every(({ emergencyOverrides }) => emergencyOverrides),
        ).toBe(true);
        expect(modes.transitionRule).toContain(
            "may not increase effective authority",
        );
        expect(
            await readText("docs/security/foundation-v1-security-review.md"),
        ).toContain("Emergency-control interpretation");
    });

    it("J0.12 R shutdown restart and lifecycle integrity reject new work and leave no authorized orphan processing", async () => {
        for (const path of [
            "scripts/stop.mjs",
            "scripts/check-stopped.mjs",
            "scripts/start.mjs",
        ])
            expect(await exists(path)).toBe(true);
        const architecture = await readText(
    ...
    it("J0.12 T all J0.1 through J0.11 gates plus the fail-closed J0.12 verifier are required together", async () => {
        for (const path of [
            ...
            "scripts/j10-go.mjs",
            "scripts/j11-go.mjs",
            "tests/acceptance/j04-gates.json",
            "tests/acceptance/j05-gates.json",
            "tests/acceptance/j06-gates.json",
            "tests/acceptance/j07-gates.json",
            "tests/acceptance/j08-gates.json",
            "tests/acceptance/j09-gates.json",
            "tests/acceptance/j10-gates.json",
            "tests/acceptance/j11-gates.json",
            "scripts/j12-foundation-v1-go.mjs",
        ])
            expect(await exists(path)).toBe(true);
        const catalog = await readJson<{ gates: string[] }>(
            "tests/acceptance/j12-gates.json",
        );
