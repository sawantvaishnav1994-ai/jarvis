...
it("accepts reviewed append-only baseline SQL", async () => {
    ...
    expect(migrations.at(-1)?.file).toBe("0009_memory_controls_audit.sql");
});
it("refuses modified, unlisted and destructive migrations", async () => {
    ...
    try {
        ...
        await expect(migrationFiles(directory)).rejects.toThrow("MIGRATION_CHECKSUM_MISMATCH");
        ...
        await expect(migrationFiles(directory)).rejects.toThrow("DESTRUCTIVE_MIGRATION_REQUIRES_RECOVERY_GATE");
        await writeFile(join(directory, "0002_unlisted.sql"), "SELECT 1;");
        await expect(migrationFiles(directory)).rejects.toThrow("INVALID_MIGRATION_MANIFEST");
    } finally {
